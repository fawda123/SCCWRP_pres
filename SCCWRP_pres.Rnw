...
\onslide<+->
<<ibi_comb, echo=F, fig.width=6.5, fig.height=3, width = '\\textwidth'>>=
combn.dat <- expand.grid(rep(list(c(1,3,5)),12))
tab.vals <- table(rowSums(combn.dat))
tab.vals <- data.frame(tab.vals)

p1<-ggplot(tab.vals,aes(x=factor(Var1), y=Freq, fill=Freq)) +
  geom_bar(stat="identity") +
  theme_mine() +
  scale_fill_gradientn(colours = pal(5)[3:5]) +
  theme(
    axis.title.x=element_text(vjust=0,size=15),
    axis.title.y=element_text(vjust=0.25,size=15,angle=90),
  	legend.position='none') +
	scale_x_discrete('IBI score') +
	scale_y_continuous('No. of combinations')
...
<<tb_map, fig = F, results = 'hide', echo = F>>=
#wes cols
cols <- pal(5)
...
p1<-ggplot(to.plo[to.plo$year %in% c('1975','1985','1995','2005'),],aes(x=month.name,y=exp(Chla_ugl),group=year,colour=year)) + 
  ...
  scale_colour_manual('Year',values=pal(5)[2:5])
...
p2<-ggplot(to.plo,aes(x=Salinity_ppt,y=exp(Chla_ugl))) + 
  ...
  scale_colour_manual('Wastewater treatment',values=pal(5)[c(3, 5)]) +
...
pdf('fig/wrtds_pieces.pdf', family = 'serif', height = 2.5, width = 7)
print(p1)
print(p2)
print(p3)
print(p4)
...
mod1 <- lm(res ~ flo + dec_time + sin(2 * pi * dec_time) + cos(2 * pi * dec_time), tomod)
mod2 <- modfit(tomod)
...
pdf('fig/wrtds_perf.pdf', family = 'serif', height = 2.5, width = 7)
print(p)
dev.off()
...
<<predvals, echo = F, message = F, fig = T, fig.width = 8.5, fig.height = 4, fig.cap = 'Predicted and observed monthly chlorophyll by segment.', out.width = '\\linewidth', cache = T>>=
...
ggplot(epc.est,aes(x=dec.time)) + 
  ...
  geom_line(aes(y=bt.md),colour=pal(num_col)[3],alpha=0.9, size=1) +
  scale_y_continuous(ylabs) +
...
<<hill, echo = F, cache = T, fig.width = 4, fig.height = 3.5>>=
load('data/salwt_grd.RData')
load('data/epc_est.RData')
...
pdf('fig/trndcomp1.pdf', height = 5, width = 9, family = 'serif')
...
{\footnotesize
Dissertation committee ad advisors: B. Vondracek, L. Hatch, D. Pereira, S. Weisberg \\~\\
...
Research staffa dn employees at the San Francisco Estuary Institute}\\~\\
\end{column}
